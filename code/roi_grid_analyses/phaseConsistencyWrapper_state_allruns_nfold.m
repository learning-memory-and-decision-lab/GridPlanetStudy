...
saveDir = [dataDir sprintf('conditions_files_allruns/stimMod_dropErrors_state_%dFold/',numFolds)];
...
for i = 1:length(slist)
    [b]=marsbar_roi_allRuns(slist(i), sprintf('stimMod_dropErrors_state_%dFold',numFolds),roi);
    sin_cos_betas(i,:)=b([2,3,5,6,8,9,11,12]);
end
%% load beta coeff if prev step is already complete
for i = 1:length(slist)
load(fullfile(dataDir,sprintf('allrunsGLMs/stimMod_dropErrors_state_%dFold',numFolds),sprintf('%d/betas_dropErrors_%s.mat',slist(i),roi)));
sin_cos_betas(i,:)=b([2,3,5,6,8,9,11,12]);
end
...
for i = 1:length(slist)
    ...
    spm_design_firstlvl_gridplanet_phA_state_allruns(slist(i),  roi, sprintf('stimMod_phA_state_%dFold',numFolds), true,numFolds,0);
    %         end
    
    %                 estimate using marsbar in our ROI
    [b]=marsbar_roi_phA_allruns_deg(slist(i),sprintf('stimMod_phA_state_%dFold',numFolds), roi);
